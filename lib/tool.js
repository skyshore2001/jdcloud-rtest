...
如果postParam格式为 {_json: obj}，则以JSON格式序列化和发送obj.
*/
function callSvr(ac, param, fn, postParam, ajaxOpt)
{
	...
	if (postParam && postParam._json) {
		ajaxOpt.contentType = "application/json;charset=utf-8";
		postParam = JSON.stringify(postParam._json);
	}
...
// 服务端返回日期格式为 '1488124800'(php:unix time) 或 '2018-05-05 00:00:00'(java) 或 '/Date(1488124800000)/'(C#)
...
一般使用toJDTable:
	...
	expect(ret).toJDTable(["id", "name"]); // 必须含有id, name两个字段
	expect(ret).toJDTable(["*id", "name"]); // 必须含有id, name两个字段且id字段必须有非空值
	expect(ret).toJDTable(["*id", "!pwd"]); // 必须含有id且值为非空, 不能包含pwd字段

 */
	validateTable: function (obj, fields) {
		assert($.isArray(fields), "*** `fields' should be array");
